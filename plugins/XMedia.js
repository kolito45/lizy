...
if (Config.WORKTYPE == 'private') {
    ...
    XTroid.addCMD({pattern: 'pinterp ?(.*)', fromMe: true, dontAddCMDList: true}, (async (message, match) => {    
        ...
        ffmpeg(location)
            ...
            .save('output.mp4')
            .on('end', async () => {
                await message.sendMessage(fs.readFileSync('output.mp4'), MessageType.video, {caption: `The X-Troid\n_Interpolated to ${match[1]} FPS_`});
            });
        return await message.client.deleteMessage(message.jid, {id: downloading.key.id, remoteJid: message.jid, fromMe: true})
    }));
    ...
    XTroid.addCMD({pattern: 'pmp4slowmo', fromMe: true, dontAddCMDList: true}, (async (message, match) => {    
        ...
        ffmpeg(location)
            ...
            .save('slowmo.mp4')
            .on('end', async () => {
                await message.sendMessage(fs.readFileSync('slowmo.mp4'), MessageType.video, {caption: 'True Slow-Motion by Xtroid'});
            });
        return await message.client.deleteMessage(message.jid, {id: downloading.key.id, remoteJid: message.jid, fromMe: true})
    }));
}
